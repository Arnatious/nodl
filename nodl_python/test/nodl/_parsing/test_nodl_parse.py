...
def test_parse_multiple_accepts_lists(mocker):
    mocker.patch('nodl._parsing._parsing._parse_element_tree')

    assert (
        nodl._parsing._parsing.parse_multiple(paths=[Path('test/nodl/test.nodl.xml')]) is not None
    )

    # Test if accepts file name as string
    assert nodl._parsing._parsing.parse_multiple(paths=['test/nodl/test.nodl.xml']) is not None
